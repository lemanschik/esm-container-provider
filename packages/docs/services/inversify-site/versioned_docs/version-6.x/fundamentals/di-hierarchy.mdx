...
When using hierarchical injection, be aware that cached bindings keep cached values from the first resolution, even if the call comes from a child container.
